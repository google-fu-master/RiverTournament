...
<!--this is the rules.md file (converted to use details/summary for accessibility) -->
...
(function() {
  ...
  searchInput.addEventListener('input', function() {
    ...
    sections.forEach(sec => {
      let found = false;
      // Search in section text and list items (includes <summary> for details)
      sec.querySelectorAll('li, summary, h3, p').forEach(el => {
...
<!-- Adaptive details/summary behavior:
     - Sections are authored with <details open> by default (desktop expanded)
     - This small script collapses them on small screens (<= MOBILE_BREAK)
-->
<script>
(function(){
  const MOBILE_BREAK = 800;
  function adaptDetails(){
    document.querySelectorAll('details.rules-collapsible').forEach(d => {
      if (window.innerWidth <= MOBILE_BREAK) {
        d.removeAttribute('open'); // collapsed on small screens
      } else {
        d.setAttribute('open',''); // expanded on larger screens
      }
    });
  }
  window.addEventListener('load', adaptDetails);
  let __resize;
  window.addEventListener('resize', function(){ clearTimeout(__resize); __resize = setTimeout(adaptDetails,150); });
})();
</script>
